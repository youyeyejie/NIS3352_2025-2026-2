...
1. 证明 $f$ 不可约。
    - **证明**：$f$ 在 $F_3$ 上的根可能为 $0, 1, 2$，则
        ...
        \end{cases} \pmod 3
        $$ 因此 $f$ 没有线性因子，也就不可约。
2. $F_9 \cong F_3[x]/(x^2 + 1)$，令 $\alpha := x + (x^2 + 1) ∈ F_3[x]/(x^2 + 1) = [x]$，以 $a+b\alpha$ 的形式表示 $F_9$ 中的元素，其中 $a,b \in F_3$。
    - **解**：
        $$
        \begin{aligned}
...
### Question 2
令 $F_8 \cong F_2[x]/(x^3+x+1)$，$\alpha := x + (x^3+x+1) \in F_2[x]/(x^3+x+1) = [x]$。
